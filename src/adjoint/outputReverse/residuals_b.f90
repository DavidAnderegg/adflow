...
!  totalsw = totalsw + volref(i, j, k) * ftang
      ...
      prefd = prefd + volref(i, j, k)*fact*fact2*sum(-(actuatorregions(&
&       iregion)%axisvec*ftmpd/pref))/pref - volref(i, j, k)*ftang*sum(&
&       actuatorregions(iregion)%celltangentials(:, ii)*ftmpd)/pref**2
    end do
