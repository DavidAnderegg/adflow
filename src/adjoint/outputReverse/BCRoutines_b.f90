...
!  differentiation of bcnswalladiabatic in reverse (adjoint) mode (with options noisize i4 dr8 r8):
...
!  differentiation of bcnswallisothermal in reverse (adjoint) mode (with options noisize i4 dr8 r8):
...
!  differentiation of bcsubsonicoutflow in reverse (adjoint) mode (with options noisize i4 dr8 r8):
!   gradient     of useful results: *rev0 *rev1 *rev2 *pp0 *pp1
...
&         temp))
      ...
      temp = pexit/ss
      if (temp .le. 0.0_8 .and. (ovg .eq. 0.0_8 .or. ovg .ne. int(ovg))&
&     ) then
        tempd = 0.0_8
      else
        tempd = ovg*temp**(ovg-1)*ww1d(i, j, irho)/ss
      end if
...
!  differentiation of bcsubsonicinflow in reverse (adjoint) mode (with options noisize i4 dr8 r8):
!   gradient     of useful results: *rev0 *rev1 *rev2 *pp0 *pp1
!                *pp2 *rlv0 *rlv1 *rlv2 *ww0 *ww1 *ww2 *(*bcdata.norm)
...
&               twothird*pp1d(i, j)
          ...
          pp1d(i, j) = 0.0_8
          temp1 = ts/ttot
          if (temp1 .le. 0.0_8 .and. (govgm1 .eq. 0.0_8 .or. govgm1 .ne.&
&             int(govgm1))) then
            tempd1 = 0.0_8
          else
            tempd1 = govgm1*temp1**(govgm1-1)*ratiod/ttot
          end if
          tsd = tsd + tempd1
          ttotd = -(temp1*tempd1)
...
!  differentiation of bceulerwall in reverse (adjoint) mode (with options noisize i4 dr8 r8):
!   gradient     of useful results: *rev0 *rev1 *rev2 *pp0 *pp1
...
!  differentiation of bcfarfield in reverse (adjoint) mode (with options noisize i4 dr8 r8):
...
&           .ne. int(temp0)))) ww2d(i, j, irho) = ww2d(i, j, irho) + &
&           temp0*temp**(temp0-1)*tempd
        pp2d(i, j) = pp2d(i, j) - temp**temp0*tempd/pp2(i, j)
        wed = wfd
...
!  differentiation of computeetot in reverse (adjoint) mode (with options noisize i4 dr8 r8):
...
!  differentiation of extrapolate2ndhalo in reverse (adjoint) mode (with options noisize i4 dr8 r8):
