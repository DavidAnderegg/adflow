...
&     ) then
        ...
        tempd = temp*r**(temp-1)*rd
      end if
      ssd = temp0*pintd + pint*tempd
      ss = pint*temp0
      acd = qned + two*ovgm1*ad
      ac = qne + two*a*ovgm1
! compute the state in the halo.
      temp0 = pexit/ss
      if (temp0 .le. 0.0_8 .and. (ovg .eq. 0.0_8 .or. ovg .ne. int(ovg))&
&     ) then
        ww1d(i, j, irho) = 0.0_8
      else
        ww1d(i, j, irho) = ovg*temp0**(ovg-1)*(pexitd-temp0*ssd)/ss
      end if
      ww1(i, j, irho) = temp0**ovg
      pp1d(i, j) = pexitd
      pp1(i, j) = pexit
      temp0 = pexit/ww1(i, j, irho)
      arg1d = gamma2(i, j)*(pexitd-temp0*ww1d(i, j, irho))/ww1(i, j, &
&       irho)
      arg1 = gamma2(i, j)*temp0
      temp0 = sqrt(arg1)
      if (arg1 .eq. 0.0_8) then
        ad = 0.0_8
      else
...
! the gas law.
          ...
          ts = a2/temp0
          temp0 = ts/ttot
          if (temp0 .le. 0.0_8 .and. (govgm1 .eq. 0.0_8 .or. govgm1 .ne.&
&             int(govgm1))) then
            ratiod = 0.0_8
          else
            ratiod = govgm1*temp0**(govgm1-1)*(tsd-temp0*ttotd)/ttot
          end if
          ratio = temp0**govgm1
...
! the entropy of the free stream.
    ...
    c0 = temp
    temp0 = winf(irho)**gammainf/pinfcorr
    if (winf(irho) .le. 0.0_8 .and. (gammainf .eq. 0.0_8 .or. gammainf &
&       .ne. int(gammainf))) then
      tempd = 0.0_8
    else
      tempd = gammainf*winf(irho)**(gammainf-1)*winfd(irho)
    end if
    s0d = (tempd-temp0*pinfcorrd)/pinfcorr
    s0 = temp0
! loop over the generic subface to set the state in the
! halo cells.
    do ii=0,isize*jsize-1
...
! substract the normal velocity of the mesh.
      temp0 = bcdata(nn)%norm(i, j, 1)
      temp = bcdata(nn)%norm(i, j, 2)
      temp1 = bcdata(nn)%norm(i, j, 3)
      qn0d = temp0*u0d + u0*bcdatad(nn)%norm(i, j, 1) + temp*v0d + v0*&
&       bcdatad(nn)%norm(i, j, 2) + temp1*w0d + w0*bcdatad(nn)%norm(i, j&
&       , 3)
      qn0 = u0*temp0 + v0*temp + w0*temp1
...
&         , 3)
        ...
        temp0 = ww2(i, j, irho)
        temp = gamma2(i, j)
        temp2 = temp0**temp/pp2(i, j)
        if (temp0 .le. 0.0_8 .and. (temp .eq. 0.0_8 .or. temp .ne. int(&
&           temp))) then
          tempd0 = 0.0_8
        else
          tempd0 = temp*temp0**(temp-1)*ww2d(i, j, irho)
        end if
        sfd = (tempd0-temp2*pp2d(i, j))/pp2(i, j)
        sf = temp2
      else
! inflow
        temp2 = bcdata(nn)%norm(i, j, 1)
        ufd = u0d + temp2*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 1)
        uf = u0 + (qnf-qn0)*temp2
        temp2 = bcdata(nn)%norm(i, j, 2)
        vfd = v0d + temp2*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 2)
        vf = v0 + (qnf-qn0)*temp2
        temp2 = bcdata(nn)%norm(i, j, 3)
        wfd = w0d + temp2*(qnfd-qn0d) + (qnf-qn0)*bcdatad(nn)%norm(i, j&
&         , 3)
        wf = w0 + (qnf-qn0)*temp2
        sfd = s0d
        sf = s0
...
! halo cell.
      ...
      temp2 = sf*cc
      if (temp2 .le. 0.0_8 .and. (ovgm1 .eq. 0.0_8 .or. ovgm1 .ne. int(&
&         ovgm1))) then
        ww1d(i, j, irho) = 0.0_8
      else
        ww1d(i, j, irho) = ovgm1*temp2**(ovgm1-1)*(cc*sfd+sf*ccd)
      end if
      ww1(i, j, irho) = temp2**ovgm1
      ww1d(i, j, ivx) = ufd
      ww1(i, j, ivx) = uf
      ww1d(i, j, ivy) = vfd
      ww1(i, j, ivy) = vf
