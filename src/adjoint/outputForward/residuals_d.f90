...
!   plus diff mem management of: w:in dw:in vol:in
  subroutine sourceterms_block_d0(nn, res, iregion, plocal, plocald)
...
!thrusttnorm = sqrt((thrustt(1)**2 + thrustt(2)**2 + thrustt(3)**2))
        temp2 = actuatorregions(iregion)%thrustvec(:, ii)*factor*&
&         actuatorregions(iregion)%thrust
        temp1 = vol(i, j, k)/pref
        ftmpd = temp2*(vold(i, j, k)-temp1*prefd)/pref
        ftmp = temp2*temp1
        ftmpd = ftmpd - actuatorregions(iregion)%swirlvec(:, ii)*&
&         actuatorregions(iregion)%thrust*factor*prefd/pref**2
...
&           +vy*ftmpd(2)+ftmp(3)*vzd+vz*ftmpd(3)) + temp1*redimd
  ...
  end subroutine sourceterms_block_d0
...
!thrusttnorm = sqrt((thrustt(1)**2 + thrustt(2)**2 + thrustt(3)**2))
        ...
        ftmpd = temp2*vol(i, j, k)*(actuatorregionsd(iregion)%thrust-&
&         temp1*prefd)/pref
        ftmp = temp2*(vol(i, j, k)*temp1)
